...
It is known, that in the phase field method, the thermodynamic consistency may be affected, when the interface width is chosen unphysically large.\cite{Karma1998, Bellemans2017} Here, the value of total interface energy is computed at every scaling step in systems with different interface width, hence there is a valid concern about a systematic error introduced during this process. However, the expressions for the model parameters in the herein used model~\cite{Moelans2008} were derived using  matched asymptotic expansions in the sharp-interface limit, which should circumvent this problem to a large degree~\cite{Karma1998}. Nonetheless, a dedicated validation described in section~\ref{subsec_PFnuclbarr_validations_IWeffect} was carried out, demonstrating that this model is suitable for this purpose.
...
The above procedure provided the total interface energy change upon the particle insertion as function of the length scale ($\Delta x_i$ in Figure~\ref{fig_domain_scaling}a). Even though it is not the particle radius as in~\eqref{eq_DG_hom_iso}, the grid spacing has the same physical dimension (length), which assures that the order of the polynomial $\Delta G(\Delta x) = a(\Delta x) - b(\Delta x)^2$, $a, b >0$ is the same as in~\eqref{eq_DG_hom_iso}.
...
Nucleation barrier in both homogeneous and heterogeneous nucleation are inversely proportional to the "bulk" driving force $\Delta G_A$ (see equations~\eqref{eq_nucl_barr_hom_2D} and~\eqref{eq_nucl_barr_het_2D}). In the above formula for shape factor~\eqref{eq_NPA_PF_formula}, the critical nucleation barrier $(\Delta G_c^*)_{het}$ obtained assuming a particular value of $\Delta G_A$ is then multiplied by $\Delta G_A$. Effectively, $\Delta G_A$ cancels out and remains a shape factor multiplied by the other constants in the formula. This makes sense, because the shape factor is a constant for the given shape, independent on the driving force. 

From the above follows, that (a) the value of $\Delta G_A$ used in the Domain scaling method can be arbitrary, and thus (b) the particular value of the critical nucleation barrier $(\Delta G_c^*)_{het}$ obtained during the Domain scaling method should be interpreted with caution. The reason is that numerically, the values of shape factor $S$ may be correct irrespective of whether the input driving force $\Delta G_A$ had physically meaningful value or not. Because the 2D space used here is an abstraction, it is in fact not clear what would be a reasonable value of $\Delta G_A$, such to provide insight coming from the value of  $(\Delta G_c^*)_{het}$. For this reason, arbitrary units were indicated on the y axis with the energy change $\Delta G$ in Figure~\ref{fig_sketch_domain_scaling_PF}b and the obtained  $(\Delta G_c^*)_{het}$ is effectively taken only as an intermediate step in computing the shape factor, not as an independent result.
...
\section{Validations}
	\subsection{Effect of interface width scaling on energy values} \label{subsec_PFnuclbarr_validations_IWeffect}
	In order to validate the thermodynamic consistency of the used phase field model, the energy values obtained from the model at different scales were compared to the expected energy value in a simple case. The system contained a single, continuous interface of length $\mathit{l}$ between two phase fields, with isotropic interface energy $\sigma$. Disregarding other energy sources, the total interface energy in the system is then simply $\mathit{l}\sigma$.
	
	The validation used one simulation result from section~\ref{subsec_PFnuclbarr_validation_ISOparticle}, where a semicircle was the expected stable equilibrium shape (see inset in Figure~\ref{fig_validation_IWeffect}). Length of the interface was numerically determined in the units of grid spacing from the zero-level contour of the phase fields difference $\eta_1(x,y)-\eta_2(x,y)$. Then, the domain scaling procedure was carried out, evaluating the total interface energy from the free energy functional $F(\eta,\nabla\eta)$ related to the phase field model parameters. Figure~\ref{fig_validation_IWeffect} clearly shows, that despite variation of the interface width across 4 orders of magnitude, the obtained values of interface energy always match exactly the analytic prediction.
	
	\begin{figure}
		\centering
		\includegraphics[width=0.7\textwidth]{validate_IW_effect_in_domain_scaling.pdf}
		\caption[Validation of model thermodynamic consistency]{Validation of the model thermodynamic consistency in total interface energy determination from the free energy functional $F(\eta,\nabla\eta)$. The value of total interface energy as function of the scaled interface width is shown for the system in the inset.}
		\label{fig_validation_IWeffect}
	\end{figure}
	...
	\subsection{Particle on a plane (isotropic interface energy)}\label{subsec_PFnuclbarr_validation_ISOparticle}
	...
	The tilted plane validation with interface having inclination-dependent interface energy proved that the implementation is working also in this more complicated case. Nevertheless, for demonstration, in Figure~\ref{fig_demo_aniso_paticle_contangs_controlled} there are shown two particles (one in (a) and (c) and the other in (b) and (d)) as they were simulated using the model incorporating all the features: inclination-dependent interface energy, the general Neumann boundary condition, and the volume concentration. The initial condition was again close to the stable shape in the given conditions, hence the expected course of the simulation was to only observe smoothening of the interface and no additional shape changes. This was accomplished and the Figures~\ref{fig_demo_aniso_paticle_contangs_controlled}c~and~\ref{fig_demo_aniso_paticle_contangs_controlled}d demonstrate that the interface inclination was indeed controlled at the domain boundary.
	
	\begin{figure}
		\centering
		\includegraphics[width=\textwidth]{demo_Wulff_on_plane.pdf}
		\caption[Demonstration of control over interface inclination on the domain boundary with anisotropic particle.]{Demonstration of control over interface inclination on the domain boundary with anisotropic particle. In (a) and (b), there are initial conditions for two simulations of a Wulff shape with 3-fold symmetry on a plane, each with a different orientation. The particular Wulff shape rotation in degrees is within the argument of the $\cos$ function indicated above (a) and (b), where are also the imposed left and right contact angles $\vartheta_L, \vartheta_R$ in the respective cases. In (c) and (d), there are the simulation results after 0.12~s of simulation time (ca 115~000~time steps), demonstrating that the interfaces are inclined.}
