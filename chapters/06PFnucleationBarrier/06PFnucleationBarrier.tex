...
Nucleation barrier in both homogeneous and heterogeneous nucleation are inversely proportional to the "bulk" driving force $\Delta G_A$ (see equations~\eqref{eq_nucl_barr_hom_2D} and~\eqref{eq_nucl_barr_het_2D}). In the above formula for shape factor~\eqref{eq_NPA_PF_formula}, the (heterogeneous) critical nucleation barrier $(\Delta G_c^*)$ obtained assuming a particular value of $\Delta G_A$ is multiplied by this very $\Delta G_A$. Effectively, $\Delta G_A$ cancels out. This makes sense, because the shape factor is a constant for the given shape, independent on the driving force. 

From the above follows, that (a) the value of $\Delta G_A$ used in the Domain scaling method can be arbitrary, and thus (b) the particular value of the critical nucleation barrier $(\Delta G_c^*)$ obtained during the Domain scaling method should be interpreted with caution, because numerically the values of shape factor $S$ may be correct irrespective of whether the input driving force $\Delta G_A$ had physically meaningful value or not.
