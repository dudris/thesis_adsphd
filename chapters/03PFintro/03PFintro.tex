...
\begin{equation}  \label{eq_PFintro_functional_derivative}
	\frac{\delta F}{\delta \eta} = \left[ \frac{\partial f}{\partial \eta} - \nabla\cdot\frac{\partial f}{\partial(\nabla \eta)} \right] \,
\end{equation}
where $\nabla\cdot\partial f/\partial(\nabla \eta)$ is divergence of vector field $\partial f/\partial(\nabla \eta)$ defined by relation
\begin{equation} \label{eq_PFintro_vector_field}
	\frac{\partial f}{\partial(\nabla \eta)} = \frac{\partial f}{\partial(\partial_x\eta)}\mathbf{n}_x + \frac{\partial f}{\partial(\partial_y \eta)}\mathbf{n}_y + \frac{\partial f}{\partial(\partial_z \eta)}\mathbf{n}_z
\end{equation}
...
When applied to the above functional, it will result in the Allen-Cahn equation 
\begin{align}
...
\section{Allen-Cahn equation for two-phase system including anisotropic interface energy} \label{sec_intro_anisoIE_in_PF}
...
\section{Allen-Cahn equations for multi-phase systems}
	...
	This study emphasized, that probably the largest challenge for both MCF and MPF models are the ghost phases. These manifest as a finite amount of a third phase within an interface between two other fields. Ghost phases are usually unphysical and their amount was correlated with the (poorer) quality of match of dihedral angles in the triple junctions benchmark problems~\cite{Daubner2023}. 
...
\section{Benchmarking of phase field models}\label{sec_benchmarking_in_PF_discussed}
