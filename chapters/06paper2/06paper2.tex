...
\section{Fundamentals}\label{sec_NPA_Fundamentals}
	...
	\end{equation}
	with local interface normal angle $\theta$ and local mean curvature $K$. The interface stiffness $\tilde{\sigma}(\theta)$ is the thermodynamic driving force for the capillary motion and the local curvature $K$ is the thermodynamic coordinate. The interface stiffness is 
	...
	\begin{figure}[h]
		\centering
		\includegraphics[width=0.6\textwidth, page=2]{combine_graphs.pdf}
	...
	\end{equation}
	where $k$ is the Boltzmann constant, $T$ absolute temperature and $\Delta G_c^*$ is the \textit{critical nucleation barrier} (in J), which is to be overcome by the thermal fluctuations in order to form a critical nucleus. A nucleus with the critical area $A_c$ is unstable. The nucleation barrier $\Delta G_c^*$ is the \textit{nucleation work}~\cite{Milchev2002} to be done in order to insert a critical nucleus. Subcritical nuclei dissolve, the supercritical ones grow.
...
Figure~\ref{fig_MC_deposits_ori_ev_3} presents the case of very high nucleation barrier $\beta=90$, again in the initial condition IC\_H (and $\delta=0.5,\,n=4$). There occurs nearly no nucleation in the isotropic nucleation scenario (on average less than 1 event per simulation), so it is identical to the no-nucleation scenario (shown in Figure~\ref{fig_MC_deposits_ori_ev_3}a-b). As can be seen, the lowest-available-energy grains overtake as before, but there is no mechanism to introduce orientations absent in the seed layer. In the anisotropic nucleation scenario (Figure~\ref{fig_MC_deposits_ori_ev_3}c-d), still about $10^3$ of nucleation events took place despite the very high nucleation barrier $\beta$. That was possible because of she shape factor anisotropy - the local nucleation barrier $S(\alpha_1,\alpha_2)\beta$ was small enough for some orientations. However, the introduction of low-energy orientations did not take place, so the nucleation only supported the grains with initially-present lowest interface energy in the growth competition, 
	...
	Before the comparison to the experimental results, the most relevant assumptions on the model are briefly recalled here to put the comparison in an adequate context. Very importantly, the results were produced accepting the fundamental assumptions of the Classical Nucleation Theory, which were reviewed in Section~\ref{sec_Nucleation_fundamentals}. Hence, this model is not suitable for interpretation of systems with large driving forces/large deposition rates/low nucleation barriers, because this possibly implies small nuclei composed of units or tens of atoms, which can hardly be seen as continuum. Additionally, it is not clear to what extent it would be (in)correct to assume equilibrium shapes of nuclei generated on an off-equilibrium advancing interface.
	
	The shape factor-orientation maps and the new Wulff shape solutions are valid for only a single type of anisotropy function $h(\theta)=1+\delta\cos(n\theta)$, which seems to be suitable for simulation of solid-liquid interface energy anisotropy (demonstrated e.g. in phase field simulations of solidification). 
	
	The Monte Carlo model involves the growth of a deposit coupled with nucleation. It assumes only a single source of anisotropy in the whole deposition process, the interface energy anisotropy, which manifests in different ways in the growth and nucleation. This is a very important point when comparing to any experiment, because the real synergistic interplay of the fundamental texture forming factors (see Section~\ref{sec_microstructure_formation}) is vastly more complicated.
	
	Additionally, the real anisotropy of interface energy is very hard to obtain in practical systems, which further limits the level of detail in the comparison. 
	
	Despite all these conceptual limitations, the presented theory can be used to qualitatively explain some recent experimental results by Alimadadi et al.~\cite{Alimadadi2016}. They studied the through-thickness texture evolution in Ni electrodeposited on amorphous substrate in four samples, each of which exhibited different dominant fiber texture. The only variables in the deposition were pH and deposition rate, the resulting thickness was different in every sample, but around \qty{20}{\um}. 
	...
	In the other samples of Alimadadi et al., the initial texture contained the resulting dominant component, which gradually reinforced with thickness (similarly to how the sample in Figure~\ref{fig_Alimadadi_results} evolved from \qty{2}{\um} on). 
...
First, the wetting parameter $\Gamma$ was expressed as a function of the top and bottom grain orientations in order to enable the generalized Winterbottom construction for each orientation combination. Novel solutions to the stable equilibrium shape were employed in very strong anisotropies. The shape factor-orientation maps were produced and used as an input to the columnar growth simulation using the developed Monte Carlo method.

The obtained results essentially indicate that with stronger anisotropy, there is less effect of the non-dimensional homogeneous nucleation barrier on the nucleation rate. In a system with constant scalar interface energy $\sigma_0$ and temperature, that means lesser effect of the bulk driving force on nucleation behavior (see equation~\eqref{eq_def_beta_nondim_barrier}). 

On the other hand, the initial texture becomes a significant or even leading factor. Depending on the initial texture, it was shown to be possible to either have no nucleation irrespective of the driving force (should the initial texture contain only forbidden orientations) or to have large nucleation rate even with very small driving forces. The latter option was speculated to be one of mechanisms leading the abrupt change in texture in one of samples in~\cite{Alimadadi2016}, the one shown in Figure~\ref{fig_Alimadadi_results} in this paper.

Three improvements of the generalized Winterbottom construction were worked out: (i) the new solutions of stable equilibrium shapes, originating from multiple-times self-intersecting Wulff plot occurring at very strong ansiotropies, (ii) detailed description of the inverted shape solution in arbitrary orientation and (iii) the contact point stability condition for the nucleus as a function of both the top and bottom grain orientations. The latter is a strong influence on the model behavior. It masked the full orientation-shape factor map only to its part, where the nucleation is less probable, except for some areas near the edge of the stable angles interval. There, on the other hand, the nucleation barrier could be so small, that nucleation would proceed as well at very low driving forces. 

The used MATLAB codes are available in a Mendeley Data repository~\cite{Minar2023dataset}.

More quantitative or at least more applied insights might be delivered by the orientation selection algorithm with the following improvements:

\begin{enumerate}
	\item extension of the shape factor-orientation maps and the orientation sampling algorithm to 3D space
	\item inclusion of the inclination dependence of the grain boundary energy between the substrate and the particle (i.e. double Winterbottom construction)
	\item inclusion of another bulk field in the shape factor-orientation maps (e.g. supersaturation)
\end{enumerate}

With the above improvements, the orientation sampling algorithm could be applied in realistic simulations and compare e.g. effect of nucleation on texture evolution in materials of different crystallographic structure. 
