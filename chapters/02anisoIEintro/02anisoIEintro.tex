...
Once the values of the anisotropic interface energy are known in the different directions, the interface stiffness can be computed. In the work~\cite{Olmsted2009}, Olmsted carried out a large number of atomistic calculations to provide the grain boundary energy in Al and Ni (face centered cubic metals) and his results were also used to bring insight in the interface stiffness~\cite{Moore2021}. The results of Olmsted were generalized to a model with two material-specific grain boundary energies (for two distinct grain boundary configurations) allowing to compute the anisotropy in the grain boundary energy for any fcc metal using a MATLAB programme GB5DOF~\cite{Bulatov2014}.
