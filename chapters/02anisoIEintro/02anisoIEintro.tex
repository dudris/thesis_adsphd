...
In the following, only 2D cross section is discussed. See the Figure~\ref{fig_mariaux2011_Winterbottom_construction} for illustration.
\begin{figure}
	\centering
	\includegraphics[page=4,width=0.5\textwidth]{combine_graphs.pdf}
%	\includegraphics[width=0.7\textwidth]{mariaux2011_Winterbottom_construction.png}
%	\caption{Equilibrium stable shape of a particle with anisotropic interface energy on a plane according to capillary formalism. $\vec{n}_{f_s}$ is the substrate normal vector, $\vec{\xi}_{sl}$ is the capillary vector of the particle solid-liquid interface and $\Delta\gamma=\vec{n}_{f_s}\cdot\vec{\xi}_{sl}=\sigma_{sf}-\sigma_{lf}$, i.e. the change in the specific interface energy along the $s-l$ interface upon insertion of the particle in the system.}
	\caption{Illustration of the Winterbottom construction. The symbol $\Gamma = (\sigma_1-\sigma_3)/\sigma_0$.}
