% !TeX root = ../../thesis.tex
\chapter{Nucleation and growth in systems with anisotropic interface properties}\label{ch_anisoIEintro}
...
In liquid-gas or liquid-liquid interfaces, the terms surface tension or surface stress are sometimes used interchangeably. There, they are equivalent in meaning to the surface energy as defined above~\cite{Vermaak1968}. However, in solid-fluid or solid-solid interfaces the term surface stress is reserved for the reversible work carried out in elastic deformation of an existing surface with a given surface energy~\cite{Vermaak1968}. Because this thesis does not involve deformation of the interfaces in any way, it will not be discussed further.
...
When the heads of capillary vectors for all interface normal angles $\theta$ are connected (having a common starting point in the origin), so-called $\xi$-plot is obtained. It turns out, that the $\xi$-plot coincides with the Wulff shape~\cite{Hoffman1972, Kobayashi2001} (see third column in Figure~\ref{fig_types_anisofun}a). Hence, the parametric expression~\eqref{eq_xivec_2D} can be used to plot the 2D Wulff shape in Cartesian coordinates. This is entirely true as long as there are no corners or facets on the Wulff shape, in which cases the use of capillary vector for Wulff shape plotting must be appropriately modified (see below for details).
...
Two different types of singularities may occur in Wulff shapes, which must be treated in a specific way: corners and facets~\cite{Kobayashi2001, Fleck2011}. These are related to two specifics of the anisotropy function: (i) the corners are related to non-convexity of the Frank's plot $1/h(\theta)$ (non-convex means that there are two such points, which when connected by a straight line will have the connecting line passing outside of the shape) and (ii) the facets occur in anisotropy functions which are not twice differentiable, hence their interface stiffness cannot be computed.
\begin{figure}
	\centering
	\begin{tikzpicture}
		\draw node [below right, inner sep =0] (a) at (0,0) {\includegraphics[width=0.8\textwidth]{aniso_ex_convex_diff.jpg}};
		\draw node [below right, inner sep =0] (b) at (a.south west) {\includegraphics[width=0.8\textwidth]{aniso_ex_nonconvex_diff.jpg}};
		\draw node [below right, inner sep =0] (c) at (b.south west) {\includegraphics[width=0.8\textwidth]{aniso_ex_nonconvex_diff_convexified.jpg}};
		\draw node [below right, inner sep =0] (d) at (c.south west) {\includegraphics[width=0.8\textwidth]{aniso_ex_nonconvex_nondiff.jpg}};
		\draw node[inner sep=0,left] at (a.west) {(a)};
		\draw node[inner sep=0,left] at (b.west) {(b)};
		\draw node[inner sep=0,left] at (c.west) {(c)};
		\draw node[inner sep=0,left] at (d.west) {(d)};
		
 		\draw node[inner sep=0,below] at ($(a.north west) + (0.8\textwidth/2/4,0.5)$) {$h(\theta)$};
		\draw node[inner sep=0,below] at ($(a.north west) + (0.8\textwidth/2/4,0.5) + (0.8\textwidth/4,0) $) {$1/h(\theta)$};
		\draw node[inner sep=0,below] at ($(a.north west) + (0.8\textwidth/2/4,0.5) + 2*(0.8\textwidth/4,0) $) {$\vec{\xi}(\theta)$};
		\draw node[inner sep=0,below] at ($(a.north west) + (0.8\textwidth/2/4,0.5) + 3*(0.8\textwidth/4,0) $) {Wulff};
	\end{tikzpicture}
...
\begin{figure}
	\centering
	\includegraphics[width=0.5\textwidth]{Loginova2004_regularized_anisoIE_cusps.png}
...
\section{Other types of Wulff-shape-related constructions}\label{sec_types_of_Wulff_constructions}
...
\begin{figure}
	\centering
	\includegraphics[width=0.5\textwidth]{Zucker2012_double_Winterbottom.png}
...
The generalized Winterbottom construction applies to anisotropy function of the type $h(\theta)=1+\delta\cos(n\theta)$ with strong anisotropy (where the Wulff shape has corners and "ears" behind them). The Winterbottom construction is extended by including the allowed-angle parts of the "ears" in the construction, which may result in new solutions to the equilibrium stable shape, see Figure~\ref{fig_generalized_Winterbottom} for illustration.
\begin{figure}
	\centering
	\includegraphics[width=0.7\textwidth]{Bao2017_generalized_Winterbottom.png}
	\caption[Generalized Winterbottom construction]{The principle of Generalized Winterbottom construction, taken from~\cite{Bao2017}. In (a) the anisotropy function and Wulff shape with "ears", in (b) the forbidden angles were disregarded from the Wulff shape, in (c1) the solution given by regular Winterbottom construction and in (c2)-(c4) the new solutions enabled by inclusion of the allowed parts of the "ears".}
...
\section{Nucleation fundamentals} \label{sec_Nucleation_fundamentals}
...
\end{equation}
where $R$ is the universal gas constant, $T$ absolute temperature and $\Delta G_c^*$ is a \textit{critical nucleation barrier}, which is to be overcome by the thermal fluctuations in order to form a spontaneously growing nucleus. Nucleus with the critical volume $V_c$ is unstable, but further addition of atoms will make it spontaneously grow, whether atoms subtraction to spontaneously dissolve. The nucleation barrier $\Delta G_c^*$ is the \textit{nucleation work}~\cite{Milchev2002}, the maximal positive change in Gibbs free energy associated with the nucleus insertion.
...
% These equilibrium shapes are called Wulff shapes when the interface energy is anisotropic. Equilibrium shape of interface with isotropic interface energy is a sphere.
    \subsection{Nucleation with isotropic interface energy}
        ...
        \begin{align}
            (\Delta G^*_c)_{het} &= \hat{V}_{hom}S(\alpha)\frac{4\sigma^3}{\Delta G_v^2} \\
    ...
    and
    \begin{equation}
        (\Delta G^*_c)_{hom}^{ani} = \hat{V}_{hom}^{ani}\frac{4\sigma_0^3}{\Delta G_v^2} \,.
    \end{equation}
    ...
    The shape factor $S(\Gamma,\bm{n}_{\mathit{2}})=V_{het}^{ani}/V_{hom}^{ani}$ represents the ratio of volume of the truncated portion of the Wulff shape relative to volume of the untruncated one. It depends on the wetting condition, described by parameter
    \begin{equation}
    ...
    \begin{equation}
        \Delta G_{het}^{ani} = \Delta G_{hom}^{ani} S(\Gamma,\bm{n}_{\mathit{2}}) \,,
    \end{equation}
    ...
    \begin{equation}
        (\Delta G^*_c)_{het}^{ani} = (\Delta G^*_c)_{hom}^{ani} S(\Gamma,\bm{n}_{\mathit{2}}) \,.
    \end{equation}
    ...
    \begin{equation} 
        (\Delta G_c^*)_{hom} = \hat{A}_{hom}^{ani}\frac{\sigma^2}{\Delta G_A}\,.
    \end{equation}
...
\section{Microstructure formation in coatings}\label{sec_microstructure_formation}
