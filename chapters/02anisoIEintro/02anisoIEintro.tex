...
Be there an isothermic isolated thermodynamic system consisting of a block of a solid material in a vacuum. When the block of material is reversibly cleaved, two equal areas $A$ are created on each side of the cleavage, newly facing the vacuum. Total energy of the system increases by \textit{surface energy} (unit \unit{J}) of the cleavage areas $\Delta E_{2A}=2A\sigma$, where the symbol $\sigma$ stands for the \textit{specific surface energy} (unit \unit{J/m^2})~\cite{Milchev2002}. The total energy change upon cleavage $\Delta E_{2A}$ is equal to the reversible work done on the system. Should the system consist of the solid block within a liquid, the specific surface energy would be lower due to interactions between the liquid and solid (e.g. specific interactions of the molecules within the liquid with the surface or double layer formation in case of electrified surface).
