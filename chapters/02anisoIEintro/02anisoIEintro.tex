...
%	\includegraphics[page=4,width=0.5\textwidth]{combine_graphs.pdf}
	\begin{tikzpicture}[scale=1,font=\normalsize]
		...
		\begin{axis}[
			...
			\draw[-stealth] (axis cs:-3.4,0) -- (axis cs:-3.4,0.5) node [above] {$\bm{\hat{n}}_1$};
			\draw[-stealth] (axis cs:-1.8,0) -- (axis cs:-1.8,-0.5) node [below] {$\bm{\hat{n}}_3$};
			
			
			\draw[stealth-stealth] (axis cs:0.2,0) -- (axis cs:0.2,yshift1/2) node [right] {$\sigma_1-\sigma_3$} -- (axis cs:-0.4,yshift1);
			...
			\draw (-2.5,1.5) node [right] {liquid};
			\draw (-2.5,-2.5) node [right] {substrate};
		\end{axis}
	\end{tikzpicture}
