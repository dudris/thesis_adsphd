...
In tThe terms $(\sigma + \sigma''_{n_1})$, $(\sigma + \sigma''_{n_2})$ are components of the \textit{interface stiffness tensor} in the principal coordinates (i.e. diagonalized).
 
The interface velocity $v_\sigma$ due to capillary forces alone is usually taken as
\begin{equation}
	v_\sigma = L
\end{equation}

This is the way the \textit{capillary driving force} moves the surface.\\


 As a result, the interface stiﬀness is the key material property that relates interface curvature to interface thermodynamics and kinetics.~\cite{Du2007}
