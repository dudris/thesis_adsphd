%\documentclass[showinstructions,faculty=firw,department=mtk,phddegree=mtk]{adsphd}
%\documentclass[croppedpdf, faculty=firw,department=mtk,phddegree=mtk]{adsphd}
%\documentclass[showinstructions,faculty=firw,department=mtk,phddegree=mtk,doctoralschool=ads]{adsphd}
%\documentclass[showinstructions,showlabels,coverfontpercent=100,faculty=firw,department=mtk,phddegree=mtk]{adsphd}
%\documentclass[croppedpdf,showinstructions,faculty=firw,department=mtk,phddegree=mtk]{adsphd}
%\documentclass[print,bleed,cropmarks,faculty=firw,department=mtk,phddegree=mtk]{adsphd} % include bleed for the print service
%\documentclass[final,faculty=firw,department=mtk,phddegree=mtk]{adsphd}
%\documentclass[showinstructions,faculty=firw,department=mtk,phddegree=mtk,epub]{adsphd}

%\documentclass[print,faculty=firw,department=mtk,phddegree=mtk]{adsphd}
\documentclass[online,faculty=firw,department=mtk,phddegree=mtk]{adsphd}
%\documentclass[cam,faculty=firw,department=mtk,phddegree=mtk]{adsphd}


% !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
% !!                                                               !!
% !!  WARNING: do not remove the following lines between           !!
% !!  "%%% COVER: Settings %%%" and "%%% COVER: End settings %%%"  !!
% !!                                                               !!
% !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

%%% COVER: Settings %%%
\title{Effect of Anisotropy in Interface Energy on Nucleation During Film Growth and its Impact on Texture Evolution}

\author{Martin}{Minar}

\supervisor{Prof.~Dr.~Ir.~Nele~Moelans}{}
\president{Prof.~Dr.~Ir.~Lucie~Vandewalle}{}
\jurymember{Prof.~Dr.~Marc~Seefeldt}{}
\jurymember{Prof.~Dr.~Ir.~Kim~Vanmeensel}{}
\externaljurymember{Prof.~Dr.~Ir.~Joris~Proost}{UCLouvain, BE}
\externaljurymember{DSc.~Tamás~Pusztai, Ph.D.}{Wigner Research Centre for Physics, HU}
\externaljurymember{Dr.-Ing.~Daniel~Schneider}{Karlsruhe Institute of Technology, DE}

% Your research group within the department
% e.g. iMinds-DistriNet, Scientific Computing Group, ...
\researchgroup{NMDM}
\website{} % Leave empty to hide
\email{} % Leave empty to hide

\address{Celestijnenlaan 200A box 2402}
% \addresscity{B-3001 Leuven} % This is the default value. Note
                              % that 'B-3001 Heverlee' is _incorrect_!!
                              % /[https://www.kuleuven.be/communicatie/marketing/intranet/huisstijl/taalgebruik.html]

\date{January 2025}
\copyyear{2025}
%\udc{XXX.XX}            % UDC, deposit number and ISBN are no longer necessary.
%\depot{XXXX/XXXX/XX}    % Leave out the initial D/ (it is added
                         % automatically)
%\isbn{XXX-XX-XXXX-XXX-X}


% Set spine width:
\setlength{\adsphdspinewidth}{9mm}

%% Set bleeds
\setlength{\defaultlbleed}{7mm}
\setlength{\defaultrbleed}{7mm}

% Set custom cover page
% \setcustomcoverpage{mycoverpage.tex} % mycoverpage.tex is the default

%%% COVER: End settings %%%

% for the nomenclature (comment out if you do not use the nomencl package
\usepackage{nomencl}   % For nomenclature
\renewcommand{\nomname}{List of Symbols}
\newcommand{\myprintnomenclature}{%
  \cleardoublepage%
  \printnomenclature%
  \chaptermark{\nomname}
  \addcontentsline{toc}{chapter}{\nomname} %% comment to exclude from TOC
}
\makenomenclature%

% for the list of abbreviations (comment out if you do not use the glossaries package
\usepackage{glossaries} % For list of abbreviations
\newcommand{\glossname}{List of Abbreviations}
\newcommand{\myprintglossary}{%
  \renewcommand{\glossaryname}{\glossname}
  \cleardoublepage%
  \printglossary[title=List of Abbreviations]
  \chaptermark{\glossname}
  \addcontentsline{toc}{chapter}{\glossname} %% comment to exclude from TOC
}
\makeglossaries%


% BibLaTeX
\usepackage[utf8]{inputenc}
\usepackage{csquotes}
\usepackage[
  hyperref=auto,
  mincrossrefs=999,
  backend=biber,
  style=numeric-comp,
  sorting=none
]{biblatex}
\addbibresource{allpapers.bib}

% Fonts
\usepackage{textcomp} % nice greek alphabet
\usepackage{pifont}   % Dingbats
\usepackage{booktabs}
\usepackage{amssymb,amsthm}
\usepackage{amsmath}
\usepackage{bm}
\usepackage{siunitx}
\usepackage[version=4]{mhchem}
\usepackage{easyreview}
\usepackage{rotate}
\usepackage{tikz}
\usepackage{pgfplots}
\usetikzlibrary{calc,external,arrows,math}
\usepackage{tabularx}

\def\red{\color{red}}
\def\gr{\color{green}}
\def\bl{\color{blue}}
\def\vio{\color{violet}}
\def\bk{\color{black}}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{document}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\makefrontcoverXII

\maketitle

\frontmatter % to get \pagenumbering{roman}

\includepreface{preface}
\includeabstract{abstract}
\includeabstractnl{abstractnl}

% To create a list of abbreviations, there are 2 options
% 1. manual creation of list of abbreviations and inclusion as a chapter
%    \includeabbreviations{abbreviations}
% 2. automatic generation via the glossary package
    
\includeabbreviations{abbreviations}
%\myprintglossary

% To create a list of symbols, there are 2 options
% 1. include a manually created nomenclature as a chapter
%    \includenomenclature{nomenclature}
% 2. automatic generation via the nomencl package
%    \nomenclature[cB]{$c_B(\vec{x})$}{Characteristic function of $B$}
\input{nomenclature}
\myprintnomenclature

\tableofcontents
\listoffigures
\listoftables

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\mainmatter % to get \pagenumbering{arabic}

% Show instructions on a separate page
%\instructionschapters\cleardoublepage

\includechapter{01introduction}
\includechapter{02anisoIEintro}
\includechapter{03PFintro} 
%%%%\includechapter{04PFMoelans} % EMPTY
\includechapter{05paper1} 
\includechapter{06PFnucleationBarrier} 
\includechapter{06paper2} 
\includechapter{90conclusion}



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\appendix

\includeappendix{91functionalDerivative} 
%%%%%\includeappendix{92PFderivations} 

\includeappendix{93PFparametersDetermination} 
\includeappendix{94NumericalImplementation} 
\includeappendix{95LagrangeMultipliersPF} 
\includeappendix{96WulffProperties} 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\backmatter

%\includebibliography
% BibTex
%\bibliographystyle{acm}
%\bibliography{allpapers}
% BibLatex (comment lines above and comment out biblatex lines in preamble)
\printbibliography[title=\bibname]
%\printbibliography[title=allpapers]
%\instructionsbibliography

\includecv{curriculum}

\includepublications{publications}

\makebackcoverXII

\end{document}

