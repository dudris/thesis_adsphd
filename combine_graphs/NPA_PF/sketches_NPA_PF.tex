...
%		
%		% delete ytick labels
%		\draw[fill=white, white] ($(x1) + (0.2,-1)$) rectangle ++(0.4,-3.1) ;
%		\draw[fill=white, white] ($(x1) + (0.2,-1)$) rectangle ++(1,-0.3) ;
%		
%		\draw node[inner sep=0] at (-0.7,1) {(a)} ;
%		\draw node[left,inner sep=0] at (img.north west) {(b)} ;
%		\draw node[inner sep=0, rotate=90] at (img.west) {$\Delta G$ (a.u.)} ;
%		\draw node[inner sep=0] at (img.south) {$\Delta x$ (m)} ;
%		
%	\end{tikzpicture}
...
%	\tikzmath{ \cplX = -sin(\thl)*\R ; \cprX = -sin(\thr)*\R ; }
...
%	\draw[gray,-stealth] ($(cpl)+(0.3,0)$) arc (0:\thl:0.3);
%	\draw[gray,-stealth] ($(cpr)+(-0.3,0)$) arc (180:180+\thr:0.3);
%	\draw node[above] at (al) {$\sigma_{S}^L$};
%	\draw node[above] at (ar) {$\sigma_{S}^R$};
%	\draw (-1.2,-0.8) rectangle (1.2,1.4);
%	
%	\def\vshift{-0.3}
...
%			\addplot[domain=-pi:pi, samples=150, smooth, thick,blue] ({Wulffx1(deg(x))},{Wulffy1(deg(x))+yshift1});
%		\end{scope}
%		
%		\begin{scope}
%			% \clip (axis cs: -2,-5) rectangle (axis cs:0,0);
%			\clip (axis cs: 0,0) -- (axis cs: 0,2) -- (axis cs: 2,2) -- (axis cs: 2,-2) -- (axis cs: -2,-2) -- (axis cs: -2,0) -- cycle (axis cs: 0,0) ;
%			\addplot[domain=-pi:pi, samples=150, smooth,blue,dashed,thin] ({Wulffx1(deg(x))},{Wulffy1(deg(x))+yshift1});
%		\end{scope}
%		
%		\begin{scope}
%			\clip (axis cs: 0,5) rectangle (axis cs:2,0);
%			\addplot[domain=-pi:pi, samples=150, smooth,thick,magenta] ({Wulffx2(deg(x))},{Wulffy2(deg(x))+yshift2});    
%		\end{scope}
%		
%		\begin{scope}
%			% \clip (axis cs: 0,-5) rectangle (axis cs:2,0);
%			\clip (axis cs: 0,0) -- (axis cs: 2,0) -- (axis cs: 2,-2) -- (axis cs: -2,-2) -- (axis cs: -2,2) -- (axis cs: 0,2) -- cycle (axis cs: 0,0) ;
%			\addplot[domain=-pi:pi, samples=150, smooth,magenta,dashed, thin] ({Wulffx2(deg(x))},{Wulffy2(deg(x))+yshift2});    
%		\end{scope}
%		
%		\path coordinate (C1) at (axis cs:0,yshift1);
...
%		% \draw[blue] (B1) -- (zz);
%		% \draw[magenta] (B2) -- (zz);
%		\draw[<->,thick,blue] (axis cs:-0.2,0) -- (axis cs:-0.2,yshift1);
%		\draw[<->,thick,magenta] (axis cs:0.2,0) -- (axis cs:0.2,yshift2);
%		
%		\draw[fill,blue] (C1) circle (3pt);
%		\draw[fill,magenta] (C2) circle (3pt);
%		
...
%% results contour evolution particle on GB
%\begin{tikzpicture}
...
%% particle on GB - bezier fit of the contour
%\begin{tikzpicture}
%	\draw node[below right,inner sep=0] (lt) at (0,0) {\includegraphics[width=0.45\textwidth]{ISO_onGB_120x30_Bezier3.pdf}};
%	\draw node[below right,inner sep=0] (rt) at (lt.north east) {\includegraphics[width=0.45\textwidth]{ISO_onGB_120x90_Bezier3.pdf}};
%	\draw node[below right,inner sep=0] (lb) at (lt.south west) {\includegraphics[width=0.45\textwidth]{ISO_onGB_120x30_Bezier4.pdf}};
%	\draw node[below right,inner sep=0] (rb) at (lb.north east) {\includegraphics[width=0.45\textwidth]{ISO_onGB_120x90_Bezier4.pdf}};
%	\draw node[below right,inner sep=0] (lbb) at (lb.south west) {\includegraphics[width=0.45\textwidth]{ISO_onGB_120x30_Bezier5.pdf}};
%	\draw node[below right,inner sep=0] (rbb) at (lbb.north east) {\includegraphics[width=0.45\textwidth]{ISO_onGB_120x90_Bezier5.pdf}};
%	
%	\draw node[inner sep=0,right] at (lt.north west) {(a)};
%	\draw node[inner sep=0,right] at (rt.north west) {(b)};
%	\draw node[inner sep=0,right] at (lb.north west) {(c)};
%	\draw node[inner sep=0,right] at (rb.north west) {(d)};
%	\draw node[inner sep=0,right] at (lbb.north west) {(e)};
%	\draw node[inner sep=0,right] at (rbb.north west) {(f)};
%	
%	\draw node[inner sep=0.5cm,above] at (lt.north) {$\vartheta_L=120$\textdegree, $\vartheta_R=-30$\textdegree};
%	\draw node[inner sep=0.5cm,above] at (rt.north) {$\vartheta_L=120$\textdegree, $\vartheta_R=-90$\textdegree};
%	
%	\draw node[inner sep=-1cm,left] at (lt.west) {Bezier $n=3$};
%	\draw node[inner sep=-1cm,left] at (lb.west) {Bezier $n=4$};
%	\draw node[inner sep=-1cm,left] at (lbb.west) {Bezier $n=5$};
%\end{tikzpicture}

%% demonstration of Wulff on plane in PF simulation
\begin{tikzpicture}
%	[trim={left bottom right top},clip]
	\def\trimh{1cm}
	\draw node[below right,inner sep=0] (lt) at (0,0) {\includegraphics[width=0.45\textwidth, trim={0 0 0 \trimh}, clip]{demo_Wulff_on_plane_Lang77_Rang-48_snapshot01.pdf}};
	\draw node[below right,inner sep=0] (rt) at (lt.north east) {\includegraphics[width=0.45\textwidth, trim={0 0 0 \trimh}, clip]{demo_Wulff_on_plane_Lang105_Rang-139_snapshot01.pdf}};
	\draw node[below right,inner sep=0] (lb) at (lt.south west) {\includegraphics[width=0.45\textwidth, trim={0 0 0 \trimh}, clip]{demo_Wulff_on_plane_Lang77_Rang48_snapshot10.pdf}};
	\draw node[below right,inner sep=0] (rb) at (lb.north east) {\includegraphics[width=0.45\textwidth, trim={0 0 0 \trimh}, clip]{demo_Wulff_on_plane_Lang105_Rang-139_snapshot10.pdf}};
	...
	\draw node[inner sep=0.6cm,above] at (lt.north) {$\vartheta_L=77$\textdegree, $\vartheta_R=-48$\textdegree};
	\draw node[inner sep=0.6cm,above] at (rt.north) {$\vartheta_L=105$\textdegree, $\vartheta_R=-139$\textdegree};
	\draw node[inner sep=0.2cm,above] at (lt.north) {\small $h(\theta)=1+0.6\cos(3(\theta-105.6))$};
	\draw node[inner sep=0.2cm,above] at (rt.north) {\small $h(\theta)=1+0.6\cos(3(\theta-12))$};
\end{tikzpicture}
\end{document}
